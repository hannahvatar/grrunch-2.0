-- Replaces the free-floating "servings per recipe" the user picked on the
...
-- many servings you actually get from buying the ingredients as sold
-- (e.g. a club-size chicken breast pack yields far more than a small
-- recipe-card quantity would suggest). Servings are no longer a free
...
comment on column public.recipes.servings is
  'Real number of servings this recipe yields, based on the actual package/bulk size of its anchor ingredient(s) -- not an arbitrary user-chosen quantity.';
