-- Anabelle: "users now dont know what quantity to use" -- a real gap
-- left by the previous migration's badge/contribution split. Once the
-- deal-tag badge always shows the real flat flyer price (never
-- fragmented), a recipe using only PART of a package lost its only
-- visible signal for how much of it to actually use -- the fragmented
-- price used to imply that, even if confusingly. describeDealPackage()
-- (app/lib/unitConversion.ts) also actively hides the real quantity for
-- every deal-tagged ingredient, always showing generic "1 package X"
-- regardless of how much the recipe states, which made sense when a
-- deal's price could never reflect anything but the whole package --
-- no longer true for an opted-in fragmenting deal.
--
-- Threads fragment_by_weight into deal_tags (the recipes table's own
-- JSON, not just the source curated_deals row) so the client can tell,
-- per ingredient, whether "use the whole package" or "use exactly
-- this much" is the honest instruction -- see the client-side
-- IngredientLine.useQuantityText this enables in the same commit.
