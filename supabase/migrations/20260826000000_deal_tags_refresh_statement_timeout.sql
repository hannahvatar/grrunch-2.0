-- refresh_recipe_deal_tags() started hard-failing with "canceling
-- statement due to statement timeout" (57014) on every single call,
-- confirmed via the Postgres logs (18/18 calls over an hour, all
-- identical error, ~8s each). No data/logic change caused this -- it's
-- a nested loop over every recipe x every ingredient x every approved
-- curated_deals row (twice: exact-match tier, then keyword tier), and
-- on this project's free-tier shared compute that's apparently now
-- bumping the connection's default statement_timeout. refresh_recipe_
-- nutrition() (similar shape, one UNION ALL instead of two full-table
-- passes) still completes in ~5s, consistent with this specific
-- function being the more expensive one, not a general DB outage.
--
-- Fix: give this function its own longer statement_timeout, local to
-- its own execution only (PL/pgSQL `set local` reverts at the end of
-- the calling transaction) -- a pure timeout bump, zero behavior
-- change. If 30s isn't enough either, that points to something worse
-- than a tight default ceiling and needs further investigation.
