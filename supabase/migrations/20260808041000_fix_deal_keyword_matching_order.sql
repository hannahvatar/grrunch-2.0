-- Fixes a real bug in 20260808040000, caught immediately during
-- testing and corrected here as its own migration (migrations are
-- immutable once applied -- editing 20260808040000's file after
-- `supabase db push` already ran it locally would not re-apply it,
-- since the CLI tracks applied migrations by filename).
--
-- The bug: that migration interleaved the exact-name check and the
-- new keyword fallback inside a single per-deal loop. Since Postgres
-- doesn't guarantee row order without an ORDER BY, a deal with an
-- unrelated-but-coincidentally-matching keyword could be reached
-- BEFORE the ingredient's own real exact-match deal, letting the
-- keyword win first. Concretely: "Marcangelo Chicken Breast Souvlaki
-- kabobs" (its own real, still-active exact-match deal) got
-- mis-tagged with the Prime boneless chicken breast deal's price
-- instead, purely because "chicken breast" -- a keyword assigned to
-- the Prime deal -- is a literal substring of the kabob product's own
-- name, and that row happened to be returned first.
--
-- Fix: two full passes per ingredient. Pass 1 checks exact-name match
-- against every approved deal to completion; pass 2 (keyword
-- fallback) only runs at all if pass 1 found nothing anywhere. A real
-- exact match can now never lose to a keyword collision, regardless
-- of row order.
