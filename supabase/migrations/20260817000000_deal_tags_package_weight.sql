...
comment on function public.refresh_recipe_deal_tags() is
  'Rebuilds deal_tags and price for every recipe from scratch against the current curated_deals table, and used_in_recipe on curated_deals itself. Two full passes per ingredient (exact match, then keyword fallback) -- see compute_deal_tag_pricing() for the pricing/package_count logic. Each tag also carries original_price_source, price_estimated, fragment_by_weight, and package_weight_g (the deal''s real known package weight, or null when genuinely unknown/bulk) -- package_weight_g lets the client redo the same avg-weight-bridge package-count math server-side pricing already does, so an each-count ingredient display (e.g. "4 Pogo pups" from a 20-pack) can show the real "1 package required" instead of misreading the raw count as a package multiplier.';
