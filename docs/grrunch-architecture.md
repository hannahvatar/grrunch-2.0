...
13. **Every recipe must include a produce/vegetable component.** A recipe built around a protein alone (e.g. "Herb Butter Roasted Chicken Breast" with no side) isn't a complete meal, regardless of how well it prices. Recipe generation must always add at least one produce ingredient — a real one, not a garnish — even when the anchor deal itself has nothing to do with produce. Caught when a chicken-breast-only recipe was flagged as incomplete; fixed by adding a roasted broccoli side, which happened to also demonstrate the pricing chain working end to end (StatCan's real "Broccoli" price, $3.92/head, matched correctly over a less-specific produce_reference_prices entry because the unit basis was compatible — see item 12's sibling migrations for the scaling logic).
14. **Produce Reference Gaps are deals, not just reference prices.** An approved row in the "Produce Reference Gaps" Airtable table is a real, current flyer deal (see item 9's `resolve_produce_gaps` sync) — it's pushed into `curated_deals` alongside every other approved deal, not kept as a passive number only `matchReferencePrice` can see. Two consequences:
    - **Recipe generation** should draw its produce component from `curated_deals` ∪ approved Produce Reference Gaps equally — both are "this week's real deals," just reviewed through two different Airtable tables because StatCan/human-sourced produce pricing needs a different research step than a flyer's own price/regular-price pair.
    - **Grocery list display** must not fragment a produce-reference-gap-sourced ingredient into the generic `estimatedPrice` ("~$X avg.") line the way a true rounding-out staple does. It needs the same deal-tile treatment as any other `curated_deals` item — image, price, discount badge, grouped under its store. Since `refresh_recipe_deal_tags` tags an ingredient purely by name-word matching (item 11), this falls out of using the deal's own `item_name` (or a superset of its significant words) as the recipe's ingredient name — the same convention already used for anchor proteins (e.g. "Marcangelo Chicken Breast Souvlaki kabobs"). Practically, this puts single-recipe fresh produce (peppers, broccoli, a melon) on the *"scale it up, use the whole package"* branch of item 12 rather than the *"genericize it"* branch reserved for true multi-dish bulk pantry staples (rice, oil, condiments) — a 2.5 lb bag of peppers supporting one dish is not the same category as a bag of rice spread across a dozen different recipes.
    - The **Staple Reference Gap table** stays the right source for ingredients that complete a recipe but aren't tied to any current real deal (salt, garlic, rice as a side, condiments) — those legitimately keep the plain reference-price fallback (`estimatedPrice`, "Other items" section), since there's no actual store/discount to show.
    - Applied retroactively: the Kabobs recipe's "Bell Peppers" (a fragmented 300 g guess, no deal, no image) was replaced with the literal approved deal, "NO NAME® NATURALLY IMPERFECT™ SWEET PEPPERS, 2.5 LB" (1 bag) — now shows the real $6.00/$10.41 42% discount tile under Real Canadian Superstore, and the recipe's price recalculated from $3.38 to $4.20/serving accordingly.
